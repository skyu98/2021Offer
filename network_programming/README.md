...
### 二、TCP的各种异常情况
#### 1.三次握手期间异常（假设客户端为主动打开端）
* 1.**第一次握手**失败：客户端**发送SYN，没有到达服务器**
    * 客户端按**逐渐增大的时间间隔**进行重发，累计75s后返回```ETIMEOUT```
    * 服务端无感知    
<br>

* 2.第二次握手失败：服务端**发送`SYN+ACK`，没有到达客户端：**
    * 对于客户端而言等效于第一次失败(重发)
    * 服务器超时重传后，**发送RST，关闭连接**；客户端收到后返回```ECONNREFUSED```
<br>

* 3.第三次握手失败：客户端**回复`ACK`，没有到达服务端：**
    * 对于服务端等效于第二次失败（未收到`ACK`）
...
* 4. **目的不可达**错误
    * 使用`ICMP`协议传递回来
    * 按照间隔多次重发
    * 超时后客户端`connect()`返回`ENETUNREACH`错误码
<br>
...
* 6. 客户端在**服务端`accept()`之前断开连接**
    * 两次握手之后，该连接在**半完成队列**中
    * 三次握手建立连接之后，该连接在**已完成队列**中
    * 调用`accept()`之前，可能出现**客户端断开连接**的情况
    * **完成三次握手的连接会从已完成队列中移除**，此时如果调用`accept`可能产生阻塞；
    * `accept()`调用中忽略以下错误:`EWOULDBLOCK, ECONNABORTED,EPROTO,EINTR`

#### 2. 消息传输过程中发生异常
* 1.**TCP Keep Alive**机制
    * 如果建立连接后，**长时间没有数据交互，即浪费了资源**
    * `tcp_keep_alive`可设置参数：
        * `tcp_keepalive_time`: 保活时间，默认7200s（2小时）；**该时间之后发送探测报文**
        * `tcp_keepalive_intvl`：**探测报文的间隔时间**，默认75s
        * `tcp_keepalive_probes`：**探测报文无响应本次数后**，即认定该连接已死亡，中断本次连接（错误信息通知上层）
<BR>
...
* 3. **主机断电**
    * 内核无感知
    * 不会进行挥手，对端不会收到通知
    * 对端一段时间内持续重传数据（即**目标不可达错误**）
    * **服务端重启，此时端口并未打开**；收到数据后返回`RST`
